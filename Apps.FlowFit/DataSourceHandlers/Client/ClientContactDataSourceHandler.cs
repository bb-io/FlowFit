...
public class ClientContactDataSourceHandler(
    InvocationContext invocationContext,
    [ActionParameter] ClientIdentifier clientIdentifier)
    : FlowFitInvocable(invocationContext), IAsyncDataSourceHandler
{
    public async Task<Dictionary<string, string>> GetDataAsync(DataSourceContext context,
        CancellationToken cancellationToken)
    {
        if (clientIdentifier.ClientId == null)
            throw new Exception("Please specify client ID first.");
        
        var request = new FlowFitRequest($"/api/v1/ClientContacts?ClientId={clientIdentifier.ClientId}");
